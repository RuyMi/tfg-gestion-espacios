...
fun RequestValidationConfig.bookingValidation(){
    validate<BookingDtoCreate> {
        ...
        }else if (it.startTime.isEmpty() || !it.startTime.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-5][0-9]$".toRegex())) {
        ...
        }else if( it.endTime.isEmpty()||it.endTime.matches("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-5][0-9]$".toRegex())){
            ValidationResult.Invalid("The date cannot be empty or is invalid")
        }else {
